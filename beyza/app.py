...
def get_latest_earthquakes_from_db():
...
def scrape_earthquake_data():
    ...
    if response.status_code == 200:
        ...
        # Define regex pattern to extract earthquake data
        pattern = re.compile(r'(\d{4}\.\d{2}\.\d{2}\s+\d{2}:\d{2}:\d{2}\s+\d+\.\d+\s+\d+\.\d+\s+\d+\.\d+\s+-\.-\s+\d+\.\d+\s+-\.-\s+[\w\s-]+\(?\w*\)?\s+İlksel)')

        # Find all matches of the pattern in the text
        earthquakes = pattern.findall(text)
